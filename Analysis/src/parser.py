...
def p_exp(p):
   ...
   p[0] = exp(Id(p[1]), p[2], p[3], "exp")
   #print("exp")

def p_exp2(p):
    '''exp2 : LESS'''
    p[0] = exp2(Less(p[1]), "exp2")
    #print("exp2LESS")

def p_expr2Grtr(p):
    '''exp2 : GRTR'''
    p[0] = exp2Grtr(Grtr(p[1]), "exp2")
    #print("exp2GRTR")


def p_exp2Equal(p):
    '''exp2 : EQ'''
    p[0] = exp2Equal(Eq(p[1]), "exp2Equal")
    #print("exp2EQUAL")

def p_exp2NotEq(p):
    '''exp2 : NOTEQ'''
    p[0] = exp2NotEq(notEq(p[1]), "exp2NotEqual")
    #print("exp2NOTEQU")

def p_exp2And(p):
    '''exp2 : AND'''
    p[0] = exp2And(And(p[1]), "exp2And")
    #print("exp2AND")

def p_exp2OR(p):
    '''exp2 : OR'''
    p[0] = exp2OR(Or(p[1]), "exp2Or")
    #print("exp2OR")

def p_exp2Arithemti(p):
    '''exp2 : arithmeticExp'''
    p[0] = exp2Arithmeti(p[1], "exp2Arithmetic")
    #print("exp2ArithmeticExp")

def p_arithmeticExp(p):
    '''arithmeticExp : ID EQ ID arithmeticOp ID arithmeticExp'''
    p[0] = arithmeticExp(Id(p[1]), Eq(p[2]), Id(p[3]), p[4], Id(p[5]), p[6], "arithmeticExp")
    #print("arithmeticexp")
...
def p_output(p): 
    ...
    p[0] = output(p[3], p[5], "output")
    #print("output")

def p_output2(p):
    '''output2 : ID output2'''
    p[0] = output2(Id(p[1]), p[2], "output2")
    #print("output2")
...
def p_input(p):
    ...
    p[0] = inputClass(Id(p[1]), "input")
    #print("input")

def p_funct(p):
    '''funct : FUNCTION type ID LEFTPAR type ID funct2 RIGHTPAR LEFTKEY est RIGHTKEY'''
    p[0] = funct(p[2], Id(p[3]), p[5], Id(p[6]), p[7], p[10], "funct")
    #print("funct")

def p_funct2(p):
    '''funct2 : COMMA funct type ID funct2'''
    p[0] = funct2(p[2], p[3], Id(p[4]), p[5], "funct2")
    #print("funct2")
